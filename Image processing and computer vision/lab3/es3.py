...
magnitude_filtrata = 20*np.log(cv2.magnitude(tramonto_filtrato[:,:,0],tramonto_filtrato[:,:,1])+1) #il + 1 serve per viasualizzarla

plt.subplot(2, 2, 4).axis("off") #img mostrerà range di pixel nell'img
plt.imshow(magnitude_filtrata, "gray")
plt.title("applicazione filtro")
