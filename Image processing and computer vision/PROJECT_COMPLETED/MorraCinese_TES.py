...
# DA ELIMINARE ------------------>
"""
startPointPcLayout = (40, 472)
endPointPcLayout = (255, 795)

startPointPlayerLayout = (455, 472)
endPointPlayerLayout = (672, 800)
...
def grab_frame(cap, detector):
    ...
    img = detector.findHands(frame, DebugMode)  # metodo dell'altro modulo hand_detector, disegno punti e landmark su mano
    ...
    if currentState == State._Menu:
        ...
        if gameResult != -1:
            add_immagineResult(img, gameResult, pcChoice, playerChoice)
...
def add_immagineResult(frame, gameResult, pcChoice, playerChoice):
    ...
    """ IMPLEMENTAZIONE SENZA SCREENSHOT -> visualizza immagine artificiale della scelta del giocatore
    if playerChoice == 0:
        player = sassoImg
    elif playerChoice == 1:
        player = cartaImg
    ...
    #frame[455:672, 472:800] = cv2.resize(player, (328, 217))
    """
    if gameResult != 3:
